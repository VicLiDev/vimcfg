...
" 以下符号视为单词字符（影响 * 搜索和 w/b/e 跳转）
" 注意：不要随意添加符号到 iskeyword，否则会导致意外的单词匹配。
" set iskeyword+=_,$,@,%,#,-
...
" 光标位置恢复已由 vim-lastplace 插件接管（见 plugins.vim），比手动 BufReadPost 更智能
" vim-lastplace 会自动忽略 git commit、fugitive、nerdtree 等临时 buffer
...
" 持久化撤销（persistent undo）：关闭vim后再打开仍可撤销，需要配合undodir使用
" 先创建目录：mkdir -p ~/.vim_runtime/temp_dirs/undodir
...
" ── 自动配对（已由 auto-pairs 插件接管，见 plugins.vim） ──
" auto-pairs 比手动 inoremap 更完善：处理缩进、引号内空格、跳转等边界情况
...
" 注意：filetype plugin indent on 已在 plugin_manager.vim 末尾执行，此处不再重复
